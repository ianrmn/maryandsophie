...
define('ZOOMY_BY_ERRNIO_VERSION', '1.6');
...
define('ZOOMY_BY_ERRNIO_OPTION_NAME_NOTIFICATION', 'errnio_notification_status');
...
function zoomy_by_errnio_deactivate() {
	...
	delete_option(ZOOMY_BY_ERRNIO_OPTION_NAME_NOTIFICATION);
}
...
function zoomy_by_errnio_uninstall() {
	...
	$plugins = zoomy_by_errnio_multiple_plugins();
...
function zoomy_by_errnio_get_first_post_image( $postID ) {
    $args = array(
    'numberposts' => 1,
    'order' => 'ASC',
...
function zoomy_by_errnio_load_client_script() {
    ...
    wp_register_script($handle, $script_url, false, ZOOMY_BY_ERRNIO_VERSION, true);
    ...
    if ( !empty($next_post) ) {
        ...
        $next_post_thumb = zoomy_by_errnio_get_first_post_image($next_post->ID);
        if ($next_post_thumb) {
    ...
    if ( !empty($prev_post) ) {
        ...
        $prev_post_thumb = zoomy_by_errnio_get_first_post_image($prev_post->ID);
        if ($prev_post_thumb) {
...
function zoomy_by_errnio_add_settings_menu_option() {
    add_menu_page (
        ...
        'Mobile Zoom Settings',  //menu title
        'manage_options',   //capability
        'errnio-options-zoom',   //menu_slug
...
function zoomy_by_errnio_add_settings_link_on_plugin($links, $file) {
    ...
    if ($file == $this_plugin) {
		$adminpage_url = admin_url( 'admin.php?page=errnio-options-zoom' );
...
function zoomy_by_errnio_admin_notice() {
	...
	$settingsurl = admin_url( 'admin.php?page=errnio-options-zoom' );

	$shouldshow = get_option(ZOOMY_BY_ERRNIO_OPTION_NAME_NOTIFICATION);

	if($hook_suffix == 'plugins.php' && $needregister && $shouldshow != 'hide'){
		$plugin_name = "Mobile Zoom";
		$message_id = "mobile-zoom-errnio";
...
function zoomy_by_errnio_close_notification() {
	add_option(ZOOMY_BY_ERRNIO_OPTION_NAME_NOTIFICATION, 'hide');
}

function zoomy_by_errnio_multiple_plugins() {
...
function zoomy_by_errnio_admin_page() {
    ...
    <div class="errnio-settings-wrap">
		...
		if (!$needregister) {
			...
			if ($tagId) {
				echo '<div id="errnioSettingsAdmin" class="errnio-settings-screen" data-tagId="'.$tagId.'" data-installName="'.ZOOMY_BY_ERRNIO_INSTALLER_NAME.'">';
...
add_action('wp_ajax_zoomy_by_errnio_close_notification', 'zoomy_by_errnio_close_notification');